...
| Scanner not working | wrong serial settings/port, serial unavailable when the port is first opened at startup | scanner warnings in logs, `serial_port` setting, `tools/test_scanner.py` | set correct serial settings, restart backend, test port directly |
...
| Frontend shows stale/wrong state | backend not reachable, API key mismatch, poll contract mismatch | browser console, `/api/ui_state`, `frontend/src/kiosk/hooks/useUiStatePolling.js` (polling lives there, not in `App.jsx`) | verify backend path and key, inspect API payload directly |
...
| Machine stuck in starting/error loop | telemetry confirmation missing, timeout settings too strict, pending state race | `events.log` for `SELECTION_TIMEOUT`/runstate events, timeout settings | tune `machine_reservation_minutes` / thresholds, verify telemetry transitions, clear root cause then retry |
